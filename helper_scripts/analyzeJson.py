...
# Configuration
folder_path = r'testfiles/batch1/' # folder path with files to analyze
...
ignore_list = ['extension','address','telecom']#['extension', 'activity','contained']  # Replace with paths to ignore
...
def analyze_json_files(folder_path, keyword, anchor_path='', ignore_list=None):
                    ...
                    # Check if the root data is a list and process accordingly
                        if isinstance(anchor_obj, list):
                            for item in anchor_obj:
                                paths, max_counts = get_all_paths(item, all_paths=None, ignore_paths=ignore_set, max_array_counts=max_anchor_array_counts)
                                for path, values in paths.items():
                                    for value in values:
                            ...
                            paths, max_counts = get_all_paths(anchor_obj, all_paths=defaultdict(list), ignore_paths=ignore_set, max_array_counts=max_anchor_array_counts)
                            for path, values in paths.items():
                                for value in values:
                                    anchor_paths[path][value] += 1
                            for path, count in max_counts.items():
...
root_result,anchor_result, max_root_array_counts,max_anchor_array_counts, ini_filename, hashmap_filename = analyze_json_files(folder_path, keyword, anchor_path, ignore_list)
